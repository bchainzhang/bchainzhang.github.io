...
- Sisi Duan and Haibin Zhang. Recent progress on BFT in the era of blockchains. To appear in Nature Science Review (NSR). (This is an invited, peer-reviewed paper which describes our own work on BFT. The maximum number of papers that the perspective can cite is 10. So the perspective only a selected number of papers and many papers end up with no citations. If you are interested in a comprehensive survey, say the paper below.)
